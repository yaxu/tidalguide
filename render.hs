...
runvis (Right pat) name namevals =
  do let n = case lookup "cycles" namevals of
         ...
         w = case lookup "width" namevals of
               ...
               Just str -> str
         cycles = [0 .. n]
     mapM_ (\cycle -> ifMissing (name ++ "_" ++ show cycle ++ ".pdf") $ visCycle (name ++ "_" ++ show cycle) (label cycle n) ((toRational cycle) `rotL` pat) 
           ) cycles 
     return $ (++ [Space]) $ map (\cycle -> Image ("",[],[("width", w)]) [] (name ++ "_" ++ (show cycle) ++ ".pdf","fig:")) cycles
...
renderSingleAudio (id, classes, namevals) contents =
  do let codeblock = CodeBlock (id, classes, []) contents
     ...
     return $ [codeblock, Para ([Link ("",[],[]) [Image ("",[],[("width", "20")]) [] ("playcirc.pdf","fig:")] (filename,"")])]
...
renderMultiAudio (id, classes, namevals) contents =
  do let codelines = lines contents
     ...
     return $ [Table [] alignments ratios [] (codes:[audios'])]
       where showAudio (filename, msg) = [Para ([Link ("",[],[]) [Image ("",[],[("width", "20")]) [] ("playcirc.pdf","fig:")] (filename,"")])]

makeAudio :: [(String,String)] -> String -> IO (String, [Inline])
makeAudio namevals code =
  do let filename = "sounds/" ++ (map switchSlashes $ show $ md5 $ C.pack code) ++ ".mp3"
